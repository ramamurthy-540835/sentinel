...
def parse_args():
    parser = argparse.ArgumentParser(description="PRISM Sentinel Code Quality Reviewer")
...
def main():
    try:
        ...
        # Write JSON report
        json_report_path = reports_dir / "code_quality_report.json"
        with open(json_report_path, "w") as f:
            json.dump({"quality_score": score, "passed_checks": passed_checks, "total_checks": total_checks, "findings": findings}, f, indent=2)
        ...
        print(f"Critical failure in code quality reviewer: {e}", file=sys.stderr)
        sys.exit(1)
